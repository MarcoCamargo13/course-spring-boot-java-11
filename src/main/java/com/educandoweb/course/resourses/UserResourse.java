...
public class UserResourse {// disponibiliza recursos web correspondente a entidade User
	...
	public ResponseEntity<User> findById(@PathVariable Long id) {
	 ...
	 // @PostMapping -->esta anottation faz um pre processamento do controlador definindo que este metodo ira receber o Post do HTTP
	...
	public ResponseEntity<User> insert(@RequestBody User obj){//deposi chamar o service (que foi enjetado)
		...
		//return ResponseEntity.ok().body(obj);// metodo para retorna um usuario pelo URL
		return ResponseEntity.created(uri).body(obj);//creat ele espera um obj do tio uri, padrão http, quando retorna o 201, espera que a resposta contenha um cabeçalho tipo location 
